  ...
  render(p: CogInfoProps): ComponentChild | null {
    ...
    return (
      <div class={CogCss.container}>
        ...
        {firstImage.epsg
          ? SideBar.renderLine('EPSG', <a href={`http://epsg.io/${firstImage.epsg}`}> {`EPSG:${firstImage.epsg}`}</a>)
          : null}
  ...
  renderCogButtons(cog: CogTiff, index: number, total: number): ComponentChild {
    ...
    return (
      <div class={CogCss.buttons}>
        {...buttons}
        <IconButton icon={MaterialIcon.Delete} title="Remove Cog" onClick={(): void => Cogs.delete(cog.source.uri)} />
      </div>
    );
  }
}
